...
const SignUpWithEmailAndPassword = async({fullname, email, password}) => {
    try {
        ...
        if(session){
            const Avatar = avatar.getInitials(fullname)
...
const SignInWithEmailAndPassword = async({ email, password }) => {
    try {
        ...
        if(session){
            const user = await GetUserInstance(session.userId);
            return { session, user }
        }
    } catch (error) {
        throw new Error(error)
...
const HandleAutoAuthenticateUser = async() => {
    try {
...
const HandleUserSignOut = async({ type }) => {
    try {
...
export { HandleAutoAuthenticateUser, HandleUserSignOut, SignInWithEmailAndPassword, SignUpWithEmailAndPassword };
